"""
Add PACO Phase 2 people to Employee Portal without changing existing portal users.

Source: NON-Needed docs/PACO Team- Phase2 .xlsx
Share file: NON-Needed docs/PACO_Phase2_User_Credentials.xlsx
"""
...
SOURCE_FILE = DOCS / "PACO Team- Phase2 .xlsx"
...
SHARE_FILE = DOCS / "PACO_Phase2_User_Credentials.xlsx"
        ...
        employees.append(
            {
                ...
                "email": get(row, "Name", "Email", "Nokia Email ID"),
                "role": "employee",
                "domain": get(row, "Domain") or "PACO",
                "product": product,
                "ddh": get(row, "DDH"),
            }
        )
...
def write_share_file(employees: list[dict]) -> None:
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "PACO Phase 2 Credentials"
    ...
    for emp in employees:
        ...
        ws.cell(row, 9).value = str(emp["employee_id"])
    ...
    widths = [14, 32, 22, 40, 14, 12, 12, 16, 14, 18]
    ...
    note["A7"] = "Newly added = created for PACO Phase 2. Already in portal = existing password, tests/scores were not changed."
...
def main() -> int:
    ...
    print(f"Loaded {len(employees)} PACO Phase 2 employees from {SOURCE_FILE.name}")
    ...
    for emp in employees:
        ...
        existing = already_in_portal(emp["employee_id"], account_map, profile_ids)
        if existing:
    ...
    print(f"Already in portal (unchanged): {sum(1 for e in share_rows if e['skip'])}")
    ...
    print("\n=== PACO Phase 2 portal add complete ===")
    print(f"Unchanged existing portal users: {sum(1 for e in share_rows if e['skip'])}")
