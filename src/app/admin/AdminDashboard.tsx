...
export default function AdminDashboard() {
  ...
  const [employeeSkillSearch, setEmployeeSkillSearch] = useState("");
  ...
  const handleConfirmSendInvite = async () => {
    ...
    try {
      const interviewConfig: any = { interviewType: inviteType, sections: {} };
      if (inviteType === "technical") {
        interviewConfig.sections = {
          overlapping: countOverlapping,
          gap: countGap,
          projects: countProjects,
          coding: countCoding
        };
      } else if (inviteType === "non-technical") {
        interviewConfig.sections = {
  ...
  const filteredEmployees = employees
    .filter(emp => {
      ...
      const skillTerm = employeeSkillSearch.trim().toLowerCase();

      const matchesEmployeeText = !searchTerm || (
        emp.full_name?.toLowerCase().includes(searchTerm) ||
        emp.employee_id?.toLowerCase().includes(searchTerm) ||
        emp.skills?.toLowerCase().includes(searchTerm)
      );

      const matchesSkillText = !skillTerm || emp.skills?.toLowerCase().includes(skillTerm);

      return matchesEmployeeText && matchesSkillText;
    })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f8fafc] via-[#f0f4ff] to-[#e2e8f0] dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 font-sans text-foreground transition-colors duration-300">
              ...
              <div className="p-6">
                {isEmployeeDataPending ? (
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 justify-between items-center shrink-0">
                        <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,14rem)_minmax(0,12rem)] gap-3 w-full">
                        ...
                        <div className="relative">
                          <input
                            ...
                            value={employeeSkillSearch}
                            onChange={(e) => setEmployeeSkillSearch(e.target.value)}
                            className="w-full rounded-xl border border-border bg-slate-50/50 dark:bg-slate-950 p-2.5 pl-3 text-xs font-semibold text-slate-700 dark:text-slate-200 outline-none focus:ring-2 focus:ring-indigo-200"
                          />
                        </div>
                      </div>
      ...
      {inviteTargetResume && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-foreground">
          <Card className="w-full max-w-xl max-h-[calc(100vh-4rem)] bg-card border border-indigo-150 shadow-2xl rounded-3xl overflow-hidden animate-scale-up">
            <div className="bg-primary text-white px-6 py-4 flex items-center justify-between">
            ...
            <div className="p-6">
              <div className="space-y-6 overflow-y-auto max-h-[calc(100vh-18rem)]">
